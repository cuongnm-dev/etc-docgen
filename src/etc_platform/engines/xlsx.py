...
def apply_transform(value: Any, transform: str, schema: dict) -> Any:
    ...
    if transform == "expand_to_column":
        # Special marker — caller (process_sheet_writes) handles list-to-column expansion
        return value
...
def process_sheet_writes(ws, sheet_schema: dict, data: dict, schema: dict, report: FillReport):
    ...
    for write in sheet_schema.get("writes", []) or []:
        ...
        if value is None:
            continue
        # Special handling: expand_to_column transform writes list items down a column
        # starting at cell_ref (e.g. D2, D3, D4...). Used for dynamic Lookups population.
        if write.get("transform") == "expand_to_column" and isinstance(value, list):
            from openpyxl.utils import column_index_from_string
            import re as _re
            m = _re.match(r"([A-Z]+)(\d+)", cell_ref)
            if m:
                col_letter, start_r = m.group(1), int(m.group(2))
                for i, item in enumerate(value):
                    target = f"{col_letter}{start_r + i}"
                    if safe_write(ws, target, str(item) if item is not None else "", sheet_schema, report):
...
def process_data_table(ws, sheet_schema: dict, data: dict, schema: dict, report: FillReport):
    ...
    # ── STEP 1b: Unhide all rows + reset outline grouping in data region ──
    # Templates may ship with Excel outline groups (rows have outline_level>0,
    # collapsed=True, hidden=True). Reset to flat layout so all data rows show.
    for rn in list(ws.row_dimensions):
        if start_row <= rn <= end_cap:
            rd = ws.row_dimensions[rn]
            rd.hidden = False
    ...
    for i, row_item in enumerate(rows_data):
        ...
        if data_rh:
            ...
            # No fixed height → enable Excel auto-fit (Excel computes row
            # height from wrap_text content on file open).
            # Setting height=None implicitly clears customHeight in OOXML.
