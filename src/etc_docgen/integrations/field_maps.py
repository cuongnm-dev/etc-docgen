...
for the skill ↔ etc-docgen contract.
...
Each doc type has:
  ...
  - export_targets: which etc-docgen export targets to use
"""

from __future__ import annotations

# ──────────────────────────────────────────────────────────────────
# Routing: which doc types use etc-docgen vs Pandoc
# ──────────────────────────────────────────────────────────────────

DOCGEN_DOC_TYPES = {"tkcs", "tkct", "tkkt", "hdsd", "xlsx"}
"""Doc types rendered by etc-docgen (has templates)."""

PANDOC_DOC_TYPES = {"de-an-cds", "hsmt", "hsdt", "du-toan", "nckt", "bao-cao-ct", "thuyet-minh"}
"""Doc types rendered by Pandoc (no etc-docgen templates)."""


def uses_docgen(doc_type: str) -> bool:
    """Check if a doc type should use etc-docgen for rendering."""
...
def get_field_map(doc_type: str) -> dict | None:
    """Get field mapping for a doc type. Returns None if not an etc-docgen type."""
...
def get_writer_prompt_context(doc_type: str) -> str:
    ...
    if not fmap:
        return f"Doc type '{doc_type}' uses Pandoc pipeline (no etc-docgen mapping)."

    lines = [
        f"## etc-docgen Field Reference: {doc_type.upper()}",
